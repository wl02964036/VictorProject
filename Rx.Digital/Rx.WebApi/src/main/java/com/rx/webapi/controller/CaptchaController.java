...
import com.rx.webapi.security.SameTextProducer;
...
public class CaptchaController {
    ...
    @GetMapping(name = "angular.captchaImage", path = "/angular/captchaImage", produces = MediaType.IMAGE_PNG_VALUE)
    @ResponseBody
    ...
    @GetMapping(name = "angular.captchaAudio", path = "/angular/captchaAudio")
    public String captchaAudio(final WebRequest request, final HttpServletResponse response) throws IOException {
        // 直接導到儲存的檔案
        String answer = getAnswer(request);
        return "redirect:/angular/wav/" + answer + ".wav";
    }

    @GetMapping(name = "angular.reloadCaptcha", path = "/angular/reloadCaptcha", produces = MediaType.TEXT_PLAIN_VALUE)
    @ResponseBody
    ...
    @GetMapping(name = "angular.captchaNumber", path = "/angular/captchaNumber",
